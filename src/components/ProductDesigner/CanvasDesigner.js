...
export default function DesignerCanvas() {
    ...
    const frontState = useRef(createEmptyState())
    const backState = useRef(createEmptyState())

    function createEmptyState() {
        return {
            image: null,
            clientFile: null,
            imgState: { x: 200, y: 260, scale: 0.5, rotation: 0 },
            text: "",
            textState: {
                x: 200,
                y: 260,
    ...
    // Verificar si el usuario ha personalizado el pullover
    const isCustomized = () => {
        return frontState.current.image !== null ||
            backState.current.image !== null ||
            frontState.current.text !== "" ||
            backState.current.text !== "";
    }

    /* ================= IMAGE URL ================= */

    const getShirtImageUrl = (side, color) => {
        ...
        if (side === "back") {
            return color.back_image_path
                ? `${ENV.API_URL}${color.back_image_path}`
                : null
        }
    ...
    const loadShirtImage = useCallback(() => {

        if (!selectedColor) return

        const url = getShirtImageUrl(activeSide, selectedColor)
        if (!url) return
    ...
    const drawCanvas = useCallback(() => {
        ...
        if (shirtImgRef.current) {
            ctx.drawImage(shirtImgRef.current, 0, 0, canvas.width, canvas.height)
        }

        // design zone
        ctx.strokeStyle = "rgba(255,255,255,0.6)"
        ctx.setLineDash([6, 6])
        ctx.strokeRect(100, 140, 200, 240)
        ctx.setLineDash([])
    ...
    }, [activeSide, selectedElement])

    useEffect(() => {
        drawCanvas()
    }, [drawCanvas, activeSide, font, textSize, textColor, selectedElement, imageScale, imageRotation])
    ...
    const handleImageUpload = (e) => {
        ...
        reader.onload = () => {
            ...
            img.onload = () => {
                ...
                const newScale = Math.min(200 / img.width, 200 / img.height, 1)
    ...
    const generatePreviewImage = async (side) => {
        const canvas = canvasRef.current
        ...
        return new Promise((resolve) => {
            previewCanvas.toBlob((blob) => {
                if (blob) {
                    ...
                    const file = new File([blob], `${side}_preview_${Date.now()}.png`, { type: 'image/png' })
                    resolve(file)
                } else {
                    resolve(null)
                }
    ...
    const createOrder = async () => {
        ...
        try {
            ...
            // Generar preview frontal si hay diseño en el frente
            if (frontState.current.image || frontState.current.text) {
                // Cambiar al frente
                setActiveSide("front")
                // Pequeña pausa para que el canvas se actualice
                await new Promise(resolve => setTimeout(resolve, 100))

                const frontPreview = await generatePreviewImage("front")
                if (frontPreview) {
                    formData.append("preview_img", frontPreview)

                }
            }

            // Generar preview trasero si hay diseño en la parte trasera
            if (backState.current.image || backState.current.text) {
                // Cambiar a la parte trasera
                setActiveSide("back")
                // Pequeña pausa para que el canvas se actualice
                await new Promise(resolve => setTimeout(resolve, 100))

                const backPreview = await generatePreviewImage("back")
                if (backPreview) {
                    formData.append("preview_img_back", backPreview)

                }
            }

            // Restaurar el lado original
            setActiveSide(currentSide)

            // Imágenes personalizadas (las originales subidas por el usuario)
            if (frontState.current.clientFile) {
                formData.append("client_img", frontState.current.clientFile)

            }

            if (backState.current.clientFile) {
                formData.append("client_img_back", backState.current.clientFile)
        ...
        <div style={{
            ...
            <div style={{
                ...
                <button
                    ...
                    style={sideBtn(activeSide === "front")}
                >
                    Front
                </button>
                <button
                    onClick={() => setActiveSide("back")}
                    style={sideBtn(activeSide === "back")}
                >
                    Back
                </button>
            </div>
            ...
            {/* Mensaje de costo adicional por doble cara */}
            {doubleSided && (
                <div style={{
                    marginBottom: 15,
                    padding: "12px 16px",
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <span style={{ fontSize: 20 }}>✨</span>
                        <span>Double-sided customization</span>
                    </div>
                    <div style={{
                        background: "rgba(255,255,255,0.2)",
                        padding: "4px 12px",
